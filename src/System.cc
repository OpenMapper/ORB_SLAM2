...
System::System(const string &strVocFile, const string &strSettingsFile,
  ...
  std::cout << "cut string is: " << strVocFile.substr(strVocFile.size() - 3)
            << std::endl;
  bool bVocLoad = false;
  if (strVocFile.compare(strVocFile.size() - 3, 3, "txt") == 0) {
    // Text file ends with *.txt
    bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);

  } else if (strVocFile.compare(strVocFile.size() - 3, 3, "bin") == 0) {
    // Binary file ends with *.bin
